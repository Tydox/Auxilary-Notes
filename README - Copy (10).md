...
ה-trade-off הראשון הוא בין תדר העבודה לבין מספר ה-cycles ב-job. אם הארכיטקטורה אינה משתנה, העלאת התדר אינה משנה את מספר ה-cycles אלא רק מקצרת כל cycle. לדוגמה, job של `296,545 cycles` נמשך `1.482725 ms` ב-`200 MHz`, אך רק `0.988483 ms` ב-`300 MHz`. מצד שני, תדר גבוה משאיר פחות זמן ל-critical path, מקשה על עמידה ב-timing ומעלה בדרך כלל את ה-dynamic power. אם שינוי הארכיטקטורה מעלה את ה-`II` מ-2 ל-3, מספר ה-cycles גדל בקירוב ל-`444,816`; במקרה כזה גם עבודה ב-`300 MHz` נמשכת כ-`1.48272 ms`, ולכן לא מתקבל שיפור אמיתי.

המספר שש אינו קשור ל-3D stacking ואינו בחירה שרירותית. פורמט bzip2 מאפשר בין שתיים לשש Huffman tables בכל block, כפי שנבדק גם בקוד המקורי ([`run_benchmark.py`, שורות 401–403](../suites/original/bm_pyflate/run_benchmark.py#L401-L403)). קלט ה-benchmark שלנו משתמש בפועל בשש tables וב-`2,966` selectors ([נתוני ה-workload](../hardware/pyflate_v2/SIMPLIFIED_HUFFMAN_PLAN.md#measured-workload-contract)). הקובץ [`huffman_find_simple.sv`](../huffman_find_simple.sv) מממש bank אחד בלבד, ואילו ה-wrapper המוצע מחזיק שישה banks כדי שלא יהיה צורך לטעון table מחדש בכל החלפת selector. בכל lookup רק bank אחד פעיל, ולכן אין כאן פענוח של שישה symbols במקביל. היתרון הוא מעבר מהיר בין tables; החיסרון הוא יותר storage, ‏comparators ו-power.

הגדלת המקביליות בתוך ה-CAM פירושה להשתמש ביותר comparators באותו זמן. במימוש שלנו `147` entries נבדקים יחד, ולכן החיפוש יכול להסתיים בסבב אחד. לדוגמה, מימוש קטן יותר עם `49` comparators יצטרך להשתמש בהם שלוש פעמים כדי לבדוק את אותה table. כך ניתן לחסוך area ו-power, אך מספר ה-cycles עולה וה-throughput יורד. הגדלת המקביליות גורמת להשפעה ההפוכה: פחות cycles, אבל יותר logic, ‏routing ו-switching.

הוספת `pipeline` פירושה להוסיף register באמצע החישוב. לדוגמה, ב-cycle הראשון ה-CAM שומר את תוצאות `147` ההשוואות, וב-cycle השני ה-priority encoder בוחר את ה-symbol ואת `match_len`. החלוקה מקצרת את העבודה שצריכה להסתיים בכל cycle ולכן עשויה לאפשר תדר גבוה יותר. החיסרון הוא שהתוצאה מגיעה מאוחר יותר ונוספים registers ו-power. במקרה שלנו החלון הבא תלוי ב-`match_len`, ולכן העיכוב עלול להגדיל את ה-`II` ולבטל את השיפור שהתקבל מהתדר הגבוה.

ה-`16 bits` הם רוחב חלון ה-lookup ולא גודל ה-bit reservoir. ה-reservoir המלא הוא ברוחב `32 bits`, שומר את ה-compressed bits ומציג בכל פעם את 16 הביטים הבאים. הגדלתו, לדוגמה ל-`64 bits`, מאפשרת לשמור יותר קלט ולהמשיך לעבוד בזמן עצירה קצרה של ה-DMA, אך אינה מאיצה את חיפוש ה-CAM ומחייבת shifter ו-register רחבים יותר.

ה-FIFO הוא רכיב אחר מה-reservoir: זהו תור בין ה-DMA למאיץ, בצד הקלט או הפלט. FIFO גדול יותר יכול לספוג עצירות קצרות של ה-DMA ולמנוע backpressure. לדוגמה, FIFO של 16 entries יכול לשמור יותר תוצאות מ-FIFO של 4 entries לפני שהמאיץ נאלץ לעצור. עם זאת, אם ה-DMA איטי באופן קבוע, גם FIFO גדול יתמלא לבסוף. הגדלתו מוסיפה storage וצריכת power, אך אינה משנה את מהירות החישוב של ה-CAM.

`Clock Gating` פירושו לעצור את ה-clock ברכיב שאינו פעיל. כך ה-registers וה-clock tree אינם מבצעים switching ללא צורך, ולכן ה-dynamic power יורד. ה-static power כמעט אינו משתנה, ונדרשת לוגיקת control נוספת. בקוד הנוכחי יש `Operand Isolation`, שמונע שינוי מיותר בכניסות של banks לא פעילים, אך אין Clock Gating פיזי. ב-FPGA נשתמש בדרך כלל ב-clock enable ייעודי. את החיסכון ב-Watts ניתן לדעת רק לאחר בחירת target device וביצוע power analysis.

מבחינת storage, כל entry דורש `47 bits`. לכן bank אחד דורש `6.909 kbit`, ושישה banks דורשים `41.454 kbit`. ה-selector memory מוסיף `8.898 kbit`, ו-32 ביטי ה-reservoir מביאים את הסכום הידוע ל-`50.384 kbit`. הוספת storage מאפשרת לשמור את כל הטבלאות ולהקטין המתנות, אך מגדילה את משאבי החומרה ואת ה-power. חשוב להדגיש ש-`kbit` מודד נפח storage ולא area פיזי של silicon. את ה-area האמיתי ניתן לקבל רק לאחר synthesis: ב-FPGA הוא יוצג ב-LUTs, ‏FFs ו-BRAMs, וב-ASIC ב-`mm²` או במספר gates. החישוב גם אינו כולל את ה-FIFOs, ה-comparators, ה-priority logic, ה-DMA וה-routing.

לסיכום, התכנון שלנו מעדיף CAM מקבילי כדי לקבל חיפוש מהיר ופשוט להבנה, גם במחיר של יותר area ו-power. יעד של `200 MHz` עם `II=2` נותן קצב תיאורטי של `100 Msymbol/s`. ששת ה-banks חוסכים טעינה מחדש של tables, ה-reservoir וה-FIFO עוזרים להתמודד עם עצירות זמניות של ה-DMA, ו-Clock Gating יכול לחסוך dynamic power בזמן idle. הערכים הסופיים של performance, ‏area ו-power ייקבעו רק לאחר synthesis ומדידה.
