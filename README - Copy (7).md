...
באופן כללי, הגדלת המקביליות והתדר יכולה לשפר את ה-performance, אך היא מגדילה את ה-area ואת צריכת ההספק ומקשה על עמידה ב-timing. הקטנת המקביליות חוסכת חומרה והספק, אך עלולה להוסיף cycles לכל symbol ולהקטין את ה-throughput. הוספת pipeline יכולה לאפשר תדר גבוה יותר, אולם במקרה שלנו קיימת תלות בין symbols עוקבים דרך `match_len`, ולכן pipeline עמוק יותר אינו מבטיח שיפור. Buffers גדולים יותר אינם מאיצים את ליבת החישוב, אך הם עוזרים לשמור עליה פעילה כאשר קצב ה-DMA אינו קבוע. לכן התכנון שנבחר הוא פשרה: CAM מקבילי מספק חיפוש מהיר וברור, בעוד שתדר היעד של `200 MHz` ו-`II=2` נבחרו כהערכה שמרנית עד לקבלת תוצאות synthesis ו-timing אמיתיות.
