...
### תרשים בלוקים של המערכת המלאה

התרשים הבא מציג את המערכת המוצעת מקצה לקצה. נתיב ה-control עובר מה-Driver אל ה-MMIO registers, ואילו נתיב המידע עובר בין ה-DRAM למאיץ באמצעות DMA. בתוך המאיץ, רכיב המעטפת מנהל את הזרם ואת בחירת ה-table, וליבת החישוב של המאיץ מבצעת את חיפוש ה-Huffman המקבילי. יש לשים לב שרק ה-CAM matcher core ממומש בקובץ ה-SystemVerilog הנוכחי; יתר רכיבי המעטפת הם חלק מה-integration המוצע.

```mermaid
flowchart LR
    subgraph CPU["Processor and Software"]
        PY["Python pyflate"]
        API["C Extension and Driver"]
        REST["Remaining bzip2 Stages"]
        PY --> API
        API --> REST
    end

    MEM["System DRAM<br/>Compressed Input and Token Output"]

    subgraph ACC["Complete Huffman Hardware Accelerator"]
        subgraph WRAP["System Wrapper - Proposed Integration"]
            MMIO["MMIO Control and Status Registers"]
            CTRL["Job Controller and Interrupt Logic"]
            DMR["DMA Read Engine and RX FIFO"]
            RES["Bit Reservoir and Barrel Shifter"]
            SEL["Huffman Table Selector"]
            TX["Token FIFO and DMA Write Engine"]
        end

        subgraph CORE["Accelerator Computation Core"]
            BANKS["Parallel CAM Matcher Banks"]
            ENC["Priority Encoder and Result Register"]
            BANKS -->|"raw matches"| ENC
        end

        MMIO -->|"configuration and START"| CTRL
        CTRL -->|"table programming"| BANKS
        CTRL -->|"selector state"| SEL
        DMR -->|"compressed bytes"| RES
        RES -->|"16-bit lookup window"| BANKS
        SEL -->|"active table"| BANKS
        ENC -->|"match length feedback"| RES
        ENC -->|"symbol and length"| TX
    end

    API -->|"AXI4-Lite or MMIO"| MMIO
    CTRL -->|"DONE, ERROR or IRQ"| API
    API -->|"buffer addresses and sizes"| MMIO
    MEM -->|"AXI4 burst reads"| DMR
    TX -->|"AXI4 burst writes"| MEM
    MEM -->|"decoded Huffman tokens"| REST

    classDef implemented fill:#d9ead3,stroke:#38761d,stroke-width:2px;
    classDef proposed fill:#fff2cc,stroke:#bf9000,stroke-width:1px;
    class BANKS,ENC implemented;
    class MMIO,CTRL,DMR,RES,SEL,TX proposed;
```

בתרשים, הרכיבים הירוקים מייצגים את ליבת החישוב הקיימת ב-RTL, והרכיבים הצהובים מייצגים את רכיבי המעטפת הנדרשים למערכת מלאה. המעבד אינו שולח כל symbol בנפרד: הוא מגדיר job דרך MMIO, וה-DMA מעביר block שלם. בסיום, המאיץ כותב את ה-Huffman tokens לזיכרון ומודיע ל-Driver באמצעות status register או interrupt; לאחר מכן `pyflate` ממשיך את שלבי ה-bzip2 שלא הואצו.
...
### סיכום ה-trade-offs הצפויים

הטבלה מסכמת כל שינוי בעזרת דוגמה קצרה והסיבתיות שמאחוריה. אלה מגמות צפויות בלבד; את הערכים בפועל ניתן לקבוע רק לאחר synthesis, ‏timing analysis ו-power analysis עבור target device מוגדר.

| שינוי בתכנון | דוגמה קצרה | היתרון והחיסרון הצפויים |
|---|---|---|
| העלאת תדר ה-clock | מעבר מ-`200 MHz` ל-`300 MHz`, כאשר הארכיטקטורה ומספר ה-cycles אינם משתנים | כל cycle קצר יותר ולכן זמן ה-job יורד מ-`1.482725 ms` ל-`0.988483 ms`. מספר ה-cycles נשאר `296,545`. החיסרון הוא פחות זמן ל-critical path, קושי גדול יותר לעמוד ב-timing ועלייה צפויה ב-dynamic power. |
| הגדלת מספר ה-cycles ב-job | ה-`II` עולה מ-2 ל-3, למשל בעקבות pipeline נוסף | עבור `148,271` symbols מספר ה-cycles גדל בקירוב מ-`296,545` ל-`444,816`. לכן גם תדר גבוה יותר לא בהכרח משפר את זמן העבודה. |
| הוספת storage לטבלאות | מעבר מ-bank אחד לשישה banks מגדיל את table storage מ-`6.909 kbit` ל-`41.454 kbit` | כל שש הטבלאות נשארות זמינות ואפשר להחליף table לפי selector בלי לטעון אותה מחדש. מצד שני גדלים ה-storage, מספר ה-comparators, ה-routing, זמן ה-configuration וה-power. הדבר אינו מאפשר לפענח שישה symbols בו-זמנית. |
| הגדלת או הקטנת המקביליות בתוך ה-CAM | `147` comparators בבת אחת, לעומת שימוש חוזר ב-`49` comparators במשך שלושה סבבים | יותר מקביליות מאפשרת לבדוק table שלם בסבב אחד, אך מגדילה logic, ‏routing ו-switching. פחות מקביליות חוסכת area ו-power, אך בדוגמה נדרשים עד שלושה סבבים ולכן ה-throughput יורד. |
| הוספת שלב `pipeline` | מוסיפים register בין וקטור התוצאות של `147` ה-comparators לבין ה-priority encoder | העבודה מתחלקת לשני מסלולים combinational קצרים יותר, ולכן ייתכן `Fmax` גבוה יותר. עם זאת, נוספים registers ו-clock power, ו-`match_len` מתקבל מאוחר יותר; מכיוון שהחלון הבא תלוי בו, ה-`II` עלול לעלות. |
| הגדלת ה-bit reservoir | הגדלה מ-`32 bits` ל-`64 bits`, כאשר חלון ה-lookup נשאר `16 bits` | ניתן לשמור יותר compressed bits ולהתגבר על הפסקה קצרה בקלט מה-DMA. החיפוש ב-CAM עצמו אינו נעשה מהיר יותר, ונדרשים register, ‏shift ו-mux רחבים יותר. |
| הוספת `Clock Gating` | עצירת ה-clock ל-bank שאינו פעיל או לכל המאיץ בזמן idle | ה-dynamic power יורד משום שה-clock tree וה-registers אינם מתחלפים ללא צורך. ה-static power כמעט אינו משתנה, ונוספת לוגיקת gating ו-control. ה-RTL הנוכחי משתמש ב-Operand Isolation, אך אינו מממש Clock Gating פיזי. |
| הגדלת עומק ה-FIFO | לדוגמה היפותטית, הגדלת output FIFO מ-4 ל-16 entries | ניתן להמשיך להפיק symbols בזמן עצירה קצרה של ה-DMA ולכן יש פחות stalls עקב backpressure. אם קצב ה-DMA הממוצע נמוך מקצב המאיץ, גם FIFO גדול יתמלא לבסוף. בנוסף נדרשים יותר storage והספק לקריאה ולכתיבה. |
| הגדלת כלל ה-storage | שישה CAM banks, ‏selector memory ו-reservoir דורשים יחד לפחות `50.384 kbit` | יותר state מקטין טעינות חוזרות ויכול לספוג הפרעות בזרם, אך מגדיל את המשאבים ואת ה-power. ‏Kbit מודד נפח storage לוגי ולא silicon area פיזי. |

#### הקשר בין frequency לבין cycles per job

ה-frequency ומספר ה-cycles ב-job הם שני דברים שונים. תדר גבוה יותר אינו משנה לבדו את מספר ה-cycles; הוא רק מקצר את משך כל cycle. מספר ה-cycles נקבע על ידי מספר ה-symbols, ה-`II`, זמן ה-setup ומספר מחזורי ההמתנה:

$$
\begin{aligned}
C_{job} &= C_{setup}+II\cdot N_{symbols}+C_{stalls} \\
T_{job} &= \frac{C_{job}}{f_{clk}}
\end{aligned}
$$

בהערכה הנוכחית אין stalls, ולכן ב-`II=2`:

$$
\begin{aligned}
C_{job} &= 3+2\cdot148{,}271=296{,}545\text{ cycles} \\
T_{job,200} &= \frac{296{,}545}{200{,}000{,}000}=1.482725\text{ ms} \\
T_{job,300} &= \frac{296{,}545}{300{,}000{,}000}=0.988483\text{ ms}
\end{aligned}
$$

לעומת זאת, אם מוסיפים pipeline וה-`II` עולה מ-2 ל-3, ובהנחה מפשטת ש-`C_setup` נשאר 3 cycles, מתקבל בקירוב:

$$
\begin{aligned}
C_{job,new} &= 3+3\cdot148{,}271=444{,}816\text{ cycles} \\
T_{job,new} &= \frac{444{,}816}{300{,}000{,}000}=1.48272\text{ ms}
\end{aligned}
$$

כלומר העלאת התדר ל-`300 MHz` אינה נותנת שיפור במקרה הזה, משום שתוספת ה-pipeline הגדילה את מספר ה-cycles באותו יחס. לכן בוחנים את השילוב של frequency, ‏`II` ו-stalls ולא את התדר בלבד.

#### מאין הגיע הערך של שש Huffman tables

המספר שש אינו הנחה שרירותית ואינו קשור ל-3D stacking. בקוד המקורי, `huffman_groups` נקרא משדה של `3 bits`, ולאחר מכן נבדק שהערך נמצא בתחום 2–6 ([`run_benchmark.py`, שורות 401–403](../suites/original/bm_pyflate/run_benchmark.py#L401-L403)). זהו הגבול של פורמט bzip2 ולא מאפיין כללי של כל Huffman decoder; גם ה-reference decoder של bzip2 דוחה ערך קטן מ-2 או גדול מ-6 ([קוד ה-reference של bzip2](https://sourceware.org/ml/bzip2-devel/2019-q2/msg00024.html)). בנוסף, ה-characterization של `interpreter.tar.bz2` מצא שה-block של benchmark זה משתמש בפועל בשש tables וב-`2,966` selectors ([נתוני ה-workload שנמדד](../hardware/pyflate_v2/SIMPLIFIED_HUFFMAN_PLAN.md#measured-workload-contract)). לכן שש הוא גם הגבול הכללי של bzip2 וגם הערך המסוים שנדרש עבור קלט ה-benchmark. לעומת זאת, `147` entries בכל table ו-`2,966` selectors הם גבולות שנמדדו עבור workload זה ואינם הגבולות הכלליים של bzip2.

בקובץ [`huffman_find_simple.sv`](../huffman_find_simple.sv) קיימת ליבת חישוב של bank אחד בלבד. ה-wrapper של שישה banks ([`huffman_find_six_table.sv`](../hardware/pyflate_v2/rtl/huffman_find_six_table.sv)) שומר את כל הטבלאות כדי שמעבר selector יהיה מיידי, אך בכל lookup רק ה-bank שנבחר מקבל `lookup_valid`. לכן שישה banks אינם מבצעים שישה פענוחים במקביל; היתרון שלהם הוא מניעת טעינה מחדש של table בכל החלפה.

#### מה פירוש הוספת pipeline

במימוש הנוכחי, המסלול עובר מ-`lookup_bits`, דרך `147` השוואות CAM ודרך ה-priority encoder, עד ל-result register. הוספת pipeline פירושה להכניס register נוסף באמצע המסלול, למשל אחרי `raw_matches`. ב-cycle הראשון נשמר וקטור ההתאמות, וב-cycle השני ה-priority encoder בוחר את ה-symbol ואת `match_len`. כך לכל שלב יש פחות logic לבצע באותו cycle, ולכן ייתכן שניתן יהיה להעלות את התדר. החיסרון הוא שהתוצאה מגיעה cycle מאוחר יותר, נדרשים registers נוספים, ובזרם Huffman יחיד אי אפשר להכין את החלון הבא לפני ש-`match_len` ידוע. מסיבה זו ה-pipeline עלול להעלות את ה-`II` ולבטל את היתרון של התדר הגבוה יותר.

#### ההבדל בין bit reservoir, ‏buffer ו-FIFO

ה-`16 bits` אינם גודל ה-reservoir אלא רוחב חלון ה-lookup שנשלח ל-CAM. ב-wrapper המלא ה-reservoir עצמו הוא register ברוחב `32 bits`: הוא שומר compressed bits, מציג את 16 הביטים הבאים ומסיר ממנו `match_len` bits לאחר פענוח. הגדלתו ל-`64 bits` מאפשרת לאגור יותר קלט, אך אינה משנה את מספר ההשוואות או את מהירות ה-CAM. ‏FIFO הוא תור נפרד שנמצא בין ה-DMA למאיץ, בצד הקלט או בצד הפלט. הגדלת FIFO מאפשרת לספוג burst או עצירה זמנית של ה-DMA, אך אינה עוזרת אם קצב ה-DMA הממוצע נמוך מקצב המאיץ. המונח buffer הוא שם כללי שיכול להתייחס ל-reservoir, ל-FIFO או ל-DMA buffer ב-DRAM, ולכן בדוח עדיף לציין במפורש באיזה רכיב מדובר.

#### Clock Gating וצריכת הספק

`Clock Gating` עוצר את אות ה-clock ברכיבים שאינם פעילים, ולכן מקטין את ה-dynamic power שנגרם מה-clock tree ומהחלפת ערכים ב-registers. הוא אינו מבטל את ה-static או leakage power של הטרנזיסטורים. בזמן שהמאיץ פעיל אין כוונה לשנות את ה-performance, אך נדרשת לוגיקת control בטוחה עבור הפעלה וכיבוי של ה-clock. ב-FPGA מקובל להשתמש ב-clock-enable ייעודי ולא לבנות gated clock באמצעות logic רגיל. בקוד הנוכחי קיים `Operand Isolation`: הקלט לבנקים שאינם פעילים נחסם כדי לצמצם switching ב-comparators, אך זה אינו Clock Gating מלא. ללא target device ו-power analysis אי אפשר להעריך את החיסכון ב-Watts.

#### Storage ב-Kbits לעומת silicon area

כל CAM entry שומר `47 bits`, ולכן נפח ה-storage הידוע מחושב כך:

$$
\begin{aligned}
B_{one\ bank} &= 147\cdot47=6{,}909\text{ bits}=6.909\text{ kbit} \\
B_{six\ banks} &= 6\cdot6{,}909=41{,}454\text{ bits}=41.454\text{ kbit} \\
B_{selectors} &= 2{,}966\cdot3=8{,}898\text{ bits}=8.898\text{ kbit} \\
B_{known} &= 41{,}454+8{,}898+32 \\
          &= 50{,}384\text{ bits}=50.384\text{ kbit}
\end{aligned}
$$

כאן `kbit` מחושב לפי `1 kbit = 1,000 bits`. הערך `50.384 kbit` כולל את ששת ה-CAM banks, את ה-selectors ואת 32 ביטי המידע של ה-reservoir. הוא אינו כולל FIFO, ‏control registers, ‏result registers, ‏comparators, ‏priority logic, ‏DMA או routing. לכן זהו storage footprint ולא area פיזי של silicon. ‏Area של ASIC נמדד בדרך כלל ב-`mm²`, ‏`µm²` או gate equivalents, ו-area של FPGA נמדד ב-LUTs, ‏FFs ו-BRAMs; ערכים אלה מתקבלים רק לאחר synthesis ו-place-and-route.

לסיכום, התכנון בודק את כל `147` ה-entries של ה-table הפעיל במקביל, ולכן החיפוש מהיר אך דורש יותר logic ו-power. ששת ה-banks מונעים טעינה מחדש של הטבלאות אך אינם מגדילים את מספר ה-symbols שמפוענחים בו-זמנית. יעד של `200 MHz` עם `II=2` נותן קצב תיאורטי של `100 Msymbol/s`; העלאת התדר תעזור רק אם אינה מוסיפה cycles או stalls. ה-reservoir וה-FIFOs מפחיתים המתנות ל-DMA, ו-Clock Gating יכול להפחית dynamic power בזמן idle. לכן נקודת האיזון בין performance, ‏storage, ‏area, ‏frequency ו-power חייבת להיבדק לבסוף באמצעות כלי implementation אמיתיים.
