...
הטבלה מסכמת את ההשפעה הצפויה של שינוי מרכיבי הארכיטקטורה. התיאורים מציגים מגמה כללית בלבד; את הערכים בפועל ניתן לקבוע רק לאחר synthesis, ‏timing analysis ו-power analysis עבור target device מוגדר.

| שינוי בתכנון | Performance | Hardware complexity ו-Area | Operating frequency | Power consumption |
|---|---|---|---|---|
| הגדלת מספר ה-CAM entries או ה-banks המקביליים | ה-throughput נשמר גבוה וניתן להחליף tables ללא טעינה מחדש | עולים, משום שנדרשים יותר storage, ‏comparators ו-routing | ה-`Fmax` עלול לרדת בגלל routing ו-fan-out רחבים יותר | ה-dynamic power צפוי לעלות, מפני שיותר לוגיקה מבצעת switching במקביל |
| הקטנת המקביליות ושימוש ב-two-level table | ה-throughput עלול לרדת וקודים ארוכים עשויים לדרוש cycle נוסף | ה-area והמורכבות של רשת ההשוואה יורדים | עשוי להיות קל יותר להגיע לתדר גבוה, אך כל symbol עלול לדרוש יותר cycles | בדרך כלל ה-power יורד משום שפחות comparators פעילים יחד |
| העלאת תדר ה-clock כאשר ה-`II` נשאר קבוע | מספר ה-symbols בשנייה עולה וזמן העבודה מתקצר | ה-area הבסיסי אינו משתנה, אך ייתכן שיידרשו pipeline registers או timing optimizations | קשה יותר לעמוד ב-timing constraints | ה-dynamic power עולה בדרך כלל |
| הוספת שלבי `pipeline` | עשויה להעלות throughput דרך תדר גבוה יותר, אך תלות ב-`match_len` עלולה להגדיל את ה-`II` | עולים בגלל registers ולוגיקת control נוספת | ה-`Fmax` עשוי לעלות משום שה-critical path מתקצר | clock power ו-register switching עולים |
| הגדלת עומק ה-FIFO | מפחיתה stalls הנגרמים מה-DMA או מ-backpressure, אך אינה מאיצה את ה-CAM עצמו | ה-area גדל וניהול ה-buffer נעשה מורכב יותר | בדרך כלל ההשפעה קטנה, כל עוד ה-FIFO ממומש ביעילות | ה-static power וה-dynamic power של ה-buffer עולים מעט |
| הוספת `Clock Gating` או `Operand Isolation` | בדרך כלל אין שינוי ב-performance בזמן שהמאיץ פעיל | נדרשת מעט לוגיקת control נוספת | בדרך כלל אין שינוי משמעותי, אם ה-gating ממומש נכון | ה-dynamic power יורד כאשר חלקים מהמאיץ אינם פעילים |

לדוגמה, כאשר כל `147` ה-entries נבדקים במקביל, ה-CAM מוצא התאמה בלי לעבור על הטבלה entry אחרי entry; לכן מספר ה-cycles לחיפוש קטן וה-performance עולה. אם מוסיפים כמה banks מקביליים כדי לשמור את כל שש ה-Huffman tables, אפשר לעבור בין tables בלי לטעון אותם מחדש, אך שכפול ה-storage, ה-comparators וה-routing מגדיל את ה-area ואת צריכת ההספק ואף עלול להקטין את ה-`Fmax`. גם העלאת תדר ה-clock משפרת ביצועים רק כאשר ה-`II` נשאר קבוע: לדוגמה, ב-`II=2`, מעבר מ-`200 MHz` ל-`300 MHz` מעלה את הקצב מ-`100` ל-`150 Msymbol/s`, משום שמתבצעים יותר cycles בכל שנייה. החיסרון הוא שלכל cycle נשאר פחות זמן, ולכן קשה יותר לעמוד ב-timing וה-dynamic power עולה בגלל switching תכוף יותר. בכיוון ההפוך, שימוש ב-two-level table מפעיל פחות comparators בכל cycle, ולכן דורש פחות area והספק ועשוי להקל על ה-timing; עם זאת, אם symbol מסוים דורש גישה נוספת, הוא נמשך יותר cycles וה-throughput יורד. הוספת שלב `pipeline`, למשל register בין ה-comparators ל-priority encoder, מקצרת כל חלק של ה-critical path ולכן עשויה לאפשר תדר גבוה יותר, אך מוסיפה registers, לוגיקת control וצריכת clock. במקרה שלנו החלון הבא תלוי ב-`match_len` של התוצאה הקודמת, ולכן pipeline עמוק עלול להגדיל את ה-`II`; לדוגמה, אם ה-`II` עולה מ-2 ל-3, צריך להעלות את התדר מ-`200 MHz` לפחות ל-`300 MHz` רק כדי להישאר בקצב של `100 Msymbol/s`. הגדלת ה-FIFO עוזרת כאשר ה-DMA נעצר לזמן קצר: הנתונים שכבר נמצאים ב-buffer מאפשרים לליבת החישוב של המאיץ להמשיך לעבוד במקום להמתין, אך היא אינה מקצרת את חישוב ה-CAM עצמו ומוסיפה storage והספק. לכן בחרנו ב-CAM מקבילי עם יעד של `200 MHz` ו-`II=2`: הוא נותן חיפוש מהיר ומימוש שקל להסביר, במחיר של area והספק גבוהים יותר. את נקודת האיזון בפועל יהיה ניתן לקבוע רק לאחר synthesis, ‏timing analysis ו-power analysis.
